 ...
 * @author Team Gather
 *
 *         This class validates that queries for events have valid time windows,
 *         coordinates, and radius fields.
 */
@Component
	...
	private void validateTimeWindow(EventsQueryData queryData, Errors errors) {
		...
		if (timeWindow < -1 || timeWindow == 0) {
			...
			errors.reject("-3", message
					+ ":The time window must be greater than 0 hours, or it can be -1 to indicate returning all events in the next year.");
		}
	}
