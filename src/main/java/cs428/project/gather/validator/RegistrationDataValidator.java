 ...
 * @author Team Gather
 * 
 *         This class validates the password, display name, email address and
 *         default search radius of RegistrationData for creation of a new user.
 *
 */
@Component
public class RegistrationDataValidator extends AbstractValidator {
	...
	private void validateDisplayName(RegistrationData userRegistrationData, Errors errors) {
		...
		if (displayName == null) {
			...
			String message = "Field invalid-" + RegistrationData.DISPLAY_NAME_FIELD_NAME;
			errors.reject("-4", message + ":The display name already exists.  Please enter another display name.");// ,
																													// "The
																													// display
																													// name
																													// already
	...
	private void validateRadius(RegistrationData queryData, Errors errors) {
		...
		if (radius <= 0 || radius > RegistrationData.MAX_RADIUS) {
			...
			errors.reject("-3", message + ":The radius value must be greater than zero but less than "
					+ RegistrationData.MAX_RADIUS + ".");
		}
	}
}
