...
/**
 * 
 * @author Team Gather This class takes a response Data Type, "T", and makes the
 *         response into pages to avoid a single REST call return too much data
 *         at once. If multiple pages of data are returned, the information of
 *         how to get the next page of data is included.
 * 
 */
public class RESTPaginatedResourcesResponseData<T> extends RESTResponseData {
	 ...
	 * 
	 * @param bindingResult:
	 *            The binding result that contains the error information form
	 *            the validator
	...
	public static <T> ResponseEntity<RESTPaginatedResourcesResponseData<T>> badResponse(BindingResult bindingResult) {
		...
		return new ResponseEntity<RESTPaginatedResourcesResponseData<T>>(
				new RESTPaginatedResourcesResponseData<T>(errorResponse.getBody()), errorResponse.getStatusCode());
	}
	 ...
	 * @param results:
	 *            The list of results of the data type, "T"
	 * @return: A paginated response based on the list of results.
	 * 
	 */
	public static <T> ResponseEntity<RESTPaginatedResourcesResponseData<T>> createResponse(HttpServletRequest request,
			List<T> results) {
		...
		if (maybe_page_size != null) {
			try {
				...
				return new ResponseEntity<RESTPaginatedResourcesResponseData<T>>(
						new RESTPaginatedResourcesResponseData("Invalid page size '" + maybe_page_size + "'"),
		...
		if (results_per_page <= 0) {
			return new ResponseEntity<RESTPaginatedResourcesResponseData<T>>(
					new RESTPaginatedResourcesResponseData(
							"Invalid page size '" + Integer.toString(results_per_page) + "': must be greater than 0."),
					HttpStatus.BAD_REQUEST);
		}
		...
		if (maybe_page_num != null) {
			try {
				...
				return new ResponseEntity<RESTPaginatedResourcesResponseData<T>>(
						new RESTPaginatedResourcesResponseData("Invalid page number '" + maybe_page_num + "'"),
						HttpStatus.BAD_REQUEST);
			}
		...
		if (page_num < 1) {
			return new ResponseEntity<RESTPaginatedResourcesResponseData<T>>(
					new RESTPaginatedResourcesResponseData(
							"Invalid page '" + Integer.toString(page_num) + "': That page number is less than 1."),
					HttpStatus.BAD_REQUEST);

		} else if (page_num > total_pages) {
			return new ResponseEntity<RESTPaginatedResourcesResponseData<T>>(
					new RESTPaginatedResourcesResponseData(
							"Invalid page '" + Integer.toString(page_num) + "': That page contains no results."),
					HttpStatus.BAD_REQUEST);
		}

		// CALCULATE OTHER FIELDS
		String previous = (page_num <= 1) ? null
				: (request.getRequestURL().toString() + "?page=" + (page_num - 1)
						+ (maybe_page_size == null ? "" : "&size=" + maybe_page_size));
		String next = (page_num >= total_pages) ? null
				: (request.getRequestURL().toString() + "?page=" + (page_num + 1)
						+ (maybe_page_size == null ? "" : "&size=" + maybe_page_size));
		...
		return new ResponseEntity<RESTPaginatedResourcesResponseData<T>>(
				new RESTPaginatedResourcesResponseData(0, results.size(), previous, next, results.subList(start, end)),
				HttpStatus.OK);
	}

	/**
	 * Constructor. Create a paginated response based on a regular response
	 * data.
	 * 
	 * @param data:
	 *            The regular response data
	 * 
	 */
	...
	/**
	 * Constructor. Create a paginated response based on parameters.
	 * 
	 * @param status:
	 *            return status
	 * @param count:
	 *            total count of the paginated response
	 * @param previous:
	 *            URL for the previous page
	 * @param next:
	 *            URL for the next page
	 * @param results:
	 *            The list of the data results
	 * 
	 */
	...
	/**
	 * Constructor. Create a paginated response based on a error message.
	 * 
	 * @param errorMessage:
