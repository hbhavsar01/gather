...
/**
 * 
 * @author Team Gather This class is the registrant controller in charge of user
 *         related rest call such as add, registration, getting registration
 *         info, and updating user info.
 * 
 */
...
public class RegistrantsController extends AbstractGatherController {
	...
	/**
	 * 
	 * This method takes user information and adds a new registrant to the
	 * database based on the received data
	 * 
	 * @param request:
	 *            This variable is the request received from the frontend
	 *            through the rest call to register the user
	 * @param rawData:
	 *            This variable is the data field of the request received from
	 *            the frontend containing email, password, and display name.
	 * @param bindingResult:
	 ...
	 *            data.
	 * @return: This method returns an http response through which the front end
	...
	/**
	 * This method receives the user id through the http request and returns the
	 * display name for that user
	 * 
	 * @param request
	 *            to get the user name of the current user
	...
	/**
	 * 
	 * This method takes the user id from the request and returns the user
	 * profile preferences to the frontend
	 * 
	 * @param request:
	 *            This variable is the request received from the frontend
	 *            through the rest call to get user profile info
	 * @param rawData:
	 *            This variable is the data field of the request received from
	 *            the frontend containing an empty array
	 * @param bindingResult:
	 *            The controller makes this variable available any time an
	 *            object is posted/put from the frontend. It will contain errors
	 *            if there was an error in binding the object. The app continues
	 ...
	 * 
	 */
	// TODO: This looks like it should just be a GET, not a PUT. We do nothing
	// with any passed in data.
	@RequestMapping(value = "/rest/registrants/info", method = RequestMethod.PUT, consumes = "application/json", produces = "application/json")
	@ResponseBody
	public ResponseEntity<RESTResourceResponseData<Registrant>> getRegistrant(HttpServletRequest request,
			@RequestBody String rawData, BindingResult bindingResult) {
		if (!authenticatedRequest(request, bindingResult))
			return RESTResourceResponseData.<Registrant> badResponse(bindingResult);
	 ...
	 * @param request:
	 *            This variable is the request received from the frontend
	 *            through the rest call to update the user profile info
	 * @param rawData:
	 *            This variable is the data field of the request received from
	 *            the frontend containing the data to be updated
	 * @param bindingResult:
	 *            The controller makes this variable available any time an
	 *            object is posted/put from the frontend. It will contain errors
	 *            if there was an error in binding the object. The app continues
	...
	@ResponseBody
	public ResponseEntity<RESTResourceResponseData<Registrant>> updateRegistrant(HttpServletRequest request,
			@RequestBody String rawData, BindingResult bindingResult) {
		if (!authenticatedRequest(request, bindingResult))
			return RESTResourceResponseData.<Registrant> badResponse(bindingResult);

		// Use registrationUpdateDataValidator instead of
		// registrationDataValidator in this case, so as to not bloat code with
		// yet another form class
		RegistrationData registrationUpdate = RegistrationData.parseIn(rawData, registrationUpdateDataValidator,
				bindingResult);
		if (bindingResult.hasErrors())
			return RESTResourceResponseData.<Registrant> badResponse(bindingResult);

		Registrant user = getUser(request);
		if (!user.validateUserDependentFields(registrationUpdate, registrantRepo, bindingResult))
			return RESTResourceResponseData.<Registrant> badResponse(bindingResult);
