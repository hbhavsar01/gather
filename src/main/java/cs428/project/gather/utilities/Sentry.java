...
class Sentry {
	...
	public static <T> void send(Class<T> klass, Event.Level level, Throwable throwable, String customMessage,
			Map<String, String> tags, Map<String, Object> extras) {
		setDsnIfNull(
				"https://3a36312fea814df3b3baf7199c43b0c8:2250fd649bfa473f91f09d397c62f19b@sentry-gatherdemo.rhcloud.com/2");
		...
		GatherEventBuilder builder = (GatherEventBuilder) (new GatherEventBuilder(throwable, customMessage)
				.addTags(tags).addExtras(extras).setLevel(level).setLogger(klass.getName()));
...
class GatherEventBuilder extends EventBuilder {
	GatherEventBuilder(Throwable throwable, String customMessage) {
		super();
		setMessage(customMessage + "\n" + throwable.getMessage()).setCulprit(throwable.getStackTrace()[0]);
	}
