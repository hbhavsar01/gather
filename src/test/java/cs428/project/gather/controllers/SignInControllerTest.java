...
public class SignInControllerTest extends ControllerTest {
	...
	public void testSignInInvalidPassword() throws JsonProcessingException {
		ResponseEntity<RESTResponseData> apiResponse = authenticateUser("existed@email.com",
				"ThisPasswordIsTooLongAndItShouldNotBeAnValidPasswordSoTheTestShouldFail", null);
		RESTResponseData responseData = apiResponse.getBody();
		assertEquals("Field invalid-password:The password length must be 64 characters or less. ",
	...
	public void testSignInUserNotExist() throws JsonProcessingException {
		...
		RESTResponseData responseData = apiResponse.getBody();
		assertEquals("Field invalid-email:The email address doesn't exist.  Please enter another email address. ",
				responseData.getMessage());
	...
	public void testSignInAlreadySignedIn() throws JsonProcessingException {
		...
		ResponseEntity<RESTResponseData> apiResponse = authenticateUser("existed@email.com", "password",
				requestEntity.getHeaders());
		RESTResponseData responseData = apiResponse.getBody();
		assertEquals("Incorrect User State. Only non-registered users can access /rest/registrants/signin ",
				responseData.getMessage());
