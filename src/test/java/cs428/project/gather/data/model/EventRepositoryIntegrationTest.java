...
public class EventRepositoryIntegrationTest {
	...
	// Event testEvent = new Event("Test Event");
	// Timestamp changeTime = new
	// Timestamp(Calendar.getInstance().getTime().getTime());
	// ChangeLog change = new ChangeLog("Description Modified", "Soccer in the
	// park. Everyone is welcome.", changeTime);
	...
	// assertEquals(testChangeLogEntry.getAdditionalInfo(), "Soccer in the park.
	// Everyone is welcome.");
	...
	public void testFindByLocationWithin() {
		...
		Location similarLocation = new Location("Test Location2", "6543 Nowhere Blvd", "Los Angeles", "CA", "90005",
				34.0498, -118.2498);
	...
	public void testFindByOccurrenceTimeWithin() {
		...
		// Set upper bound for events in the next 2 days, should return event
		// created above
		...
		// Set upper bound for events in the next 2 days, should now return no
		// events
		dt = DateTime.now().plusDays(2);
		upperBound = new Timestamp(dt.getMillis());
	...
	public void testUpdateEvent() {
		...
		Location newLocation = new Location("New Test Location", "1234 NewPlace Blvd", "San Francisco", "CA", "94530",
				134.0498, -1118.2498);
		...
		assertEquals(2, afterModification.getParticipants().size()); // 2
																		// participants
																		// now
		assertEquals(2, afterModification.getOccurrences().size()); // 2
																	// occurrences
																	// now
		assertEquals("Updated Event Name", afterModification.getName()); // event
																			// name
																			// is
																			// updated
		assertEquals("Sports", afterModification.getCategory().getName()); // category
																			// is
																			// changed
																			// to
																			// Sports
